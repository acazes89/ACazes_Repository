...
--------------------------------------------------------------------------------------------------------------------------------------------------
/* LOOPED VERSION: the section below is specific to the looped version. This extracts ALL references to every sproc/view/function on a database. 
	It has a bug somewhere from the last round of changes I made. Use Single Sproc Version for now. */
--------------------------------------------------------------------------------------------------------------------------------------------------
...
--	DECLARE @Author varchar(100) --= 'TBD' --in string insert
--	DECLARE @IsReport bit = null
--	DECLARE @IsModelTraining bit = null
--	DECLARE @Directory varchar(250) = null
--	DECLARE @SQL varchar(max) = null
--	DECLARE @ProcDB varchar(100) = DB_NAME()
--	DECLARE @Description varchar(max) = 'describe your sproc'
--	DECLARE @MaxSearchID int = (select max(CodeSearchID) from Sandbox2.dbo.CodeSearch_Searches_v1)
--	--DECLARE @CurrentDB varchar(100) = DB_NAME()  --switched to ProcDB


--THIS IS FOR LOOPED VERSION, DEACTIVATING FOR NOW

	DECLARE @CodeSearches table 
	(CodeSearchID int, --this needs to be copied from the table insert, otherwise there will be overlap between ID's
	 [ObjectID] int,
	 [Server] varchar(100), 
...
--	DECLARE @SecondUS int = charindex('_', @ProcName, charindex('_', @ProcName)+1) --take characters between first and second underscore for author inference
--	SET @Author  =  SUBSTRING(@ProcName, charindex('_', @ProcName)+1, @SecondUS - (charindex('_', @ProcName)+1))

--Write all sprocs on current DB to Searches table
...
--ALL STORED PROCEDURES:
	SELECT DISTINCT
		...
		,[ObjectDB] = @ProcDB
		,[ObjectType] = case 
			when o.type = 'P' then 'Stored Proc'
			when o.type = 'TF' then 'Function'
			when o.type = 'FN' then 'Function'
			when o.type = 'TR' then 'Trigger'
			when o.type = 'V' then 'View' end
		,'Author' = CASE when o.name like 'usp_ac%' then 'Adam Cazes'
						 when o.name like 'usp_cs%' then 'Charles Sharp'
						 when o.name like 'usp_skb%' then 'Sriram Bhagavathula'
						 when o.name like 'MV_%' then 'Martin Vincent'
						 when o.name like 'usp_blake%' then 'Blake Turney'
						 ELSE 'unknown' END
		,[ObjectDescription] = 'initial backpopulation'
		,'IsReport' = IIF(@ProcDB = 'ReportDataMarts',1,0) --there are probably a lot of report sprocs still in Sandbox, but marking them 0 for now
		,'IsModelTraining' = CASE when o.name in ('usp_ac_FraudAnomalyModel_v1', 'usp_ac_PrescreenScore1_v5', 'usp_ac_PrescreenScore2_v2', 'usp_ac_ProfitScore_V3', 'usp_ac_ConversionScore_V3', 'usp_ac_CollectionsRevenueProj_Combined_v1', 'usp_ac_CollectionsCallModel_v1', 'usp_skb_ConversionScore_v5_OffPeak_AF', 'usp_skb_ConversionScore_v5_Peak_AF', 'usp_skb_ConversionScore_v5_OffPeak_NAF', 'usp_skb_ConversionScore_v5_Peak_NAF') then 1 else 0 END
		,'InsertDate' = getdate()
		,'FullString' = (select REPLACE(REPLACE((select REPLACE(isnull(OBJECT_DEFINITION(OBJECT_ID(o.name)), @SQL), CHAR(13) + CHAR(10), '$')), '[', ''), ']', ''))	
		--,NewLineSub = (select REPLACE(isnull(OBJECT_DEFINITION(OBJECT_ID(@ProcName)), @SQL), CHAR(13) + CHAR(10), '$'))
		--@LogicString VARCHAR(MAX) = (select REPLACE(REPLACE(@NewLineSub, '[', ''), ']', ''))
		--,SecondUS = charindex('_', sysobjects.name, charindex('_', sysobjects.name)+1)
		--,Author = SUBSTRING(sysobjects.name, charindex('_', sysobjects.name)+1, charindex('_', sysobjects.name, charindex('_', sysobjects.name)+1) - (charindex('_', sysobjects.name)+1))
		,'Directory' = 'server only'
--new way:
	FROM sys.objects o
	JOIN sys.syscomments c ON o.object_id = c.id
	JOIN sys.schemas s ON o.schema_id = s.schema_id
	WHERE
    o.type IN ('P', 'V')
    AND o.is_ms_shipped = 0
--FOR TEST
	AND o.name = 'vw_Prescreen1v5_ScoreInputDate_dev' 
...
--Copy to variable table to loop through
	INSERT INTO @CodeSearches 
	 (CodeSearchID, ObjectID, [Server], ObjectName, ObjectDB, ObjectType, Author, ObjectDescription, IsReport, IsModelTraining, InsertDate, FullString)

	 Select CodeSearchID, ObjectID, [Server], ObjectName, ObjectDB, ObjectType, Author, ObjectDescription, IsReport, IsModelTraining, InsertDate, FullString
	 From Sandbox2.dbo.CodeSearch_Searches_v1
	 Where LastSearched IS NULL
	 And ObjectName = 'usp_ac_ReferenceSearch_v1' --LIMIT FOR TESTING

DECLARE @SearchesRemaining int = (select count(*) from @CodeSearches)

WHILE EXISTS (select * from @CodeSearches)
BEGIN --Open 1

DECLARE @CurrentCodeSearch int = (select top 1 CodeSearchID from @CodeSearches where LastSearched is null order by CodeSearchID) --CHANGED TO ID INSTEAD OF INSERTDATE
DECLARE @ProcName varchar(250) = (select top 1 ObjectName from @CodeSearches where CodeSearchID = @CurrentCodeSearch)


PRINT '************** CurrentCodeSearch = '+convert(varchar(50), @CurrentCodeSearch)+'  |  '+@ProcName+'********************'
PRINT 'NUM SEARCHES REMAINING = '+convert(varchar(50), @SearchesRemaining)
...
--			- added Logger search to Sections table --never mind. Let's have people review the PrimaryOutputs manually
--			- modify primary output - take the found references, remove the alias, then search for them in the last section. If they exist there, they are primary.
--6/5/23 AC	- split out alias indices subquery into a temp table, added error check for dupes and missing/incorrect index values, and added insert into separate error log table
--6/6/23 AC	- added @StartPosition to sections charindex loop. It was only checking if the keyword existing in the whole string, not the remaining string. This somehow still did not fix the issue
...
--FOR MANUAL RUNS:
	...
	--SET @ProcName = 'usp_ac_ReferenceSearch_v1_test'--'[dbo].[usp_ac_ProfitScore_V3_test]'--'dbo.usp_ac_FraudAnomalyModel_v1'--
	--SET @ProcDB = 'Sandbox'
	--DECLARE @CurrentCodeSearch int = 8
	

--DECLARE @CurrentCodeSearch int --moved outside of loop
...
DECLARE @Strings TABLE (SearchString varchar(250), SearchDB varchar(250), SearchSchema varchar(250), SearchTable varchar(250))
	...
	--where TableName = 'CashData'



--SET @ProcName = IIF(isnull(@ProcName,'')='', @Author+' | Ad-Hoc query | '+convert(varchar(200),convert(date,getdate())), @ProcName) --only needed for adhocs
...
--	 END
	...
	--NOTES: 
		--EACH table reference found is inserted, even if found multiple times. We have to do it this way, because you can have multiple tables with different aliases, so we need to search for all possible fields belonging to each mapped table for each table+alias combo. If you don't specify the alias, you run into the issue of incorrect matches due to common field names. 
		--the mapper is designed to be run on a loop, when desired, but when running a specific sproc, only 1 CodeSearchID is involved. 
	...
	--							FROM [LeadEnvy].[dbo].[vw_CSIDataRobotProfitScoreV3Outcomes] (nolock)'
	--DECLARE @Author varchar(100) = 'Adam C'
	--SET @ProcName = 'vw_Prescreen1v5_ScoreInputDate_dev'--'[dbo].[usp_ac_ProfitScore_V3_test]'--'dbo.usp_ac_FraudAnomalyModel_v1'--
	--SET @ProcDB = 'Sandbox'
	--DECLARE @NewLineSub VARCHAR(MAX) = (select REPLACE(isnull(OBJECT_DEFINITION(OBJECT_ID(@ProcName)), @SQL), CHAR(13) + CHAR(10), '$'))	--change new lines for $ in either sproc def or ad-hoc string
	--DECLARE @LogicString VARCHAR(MAX) = (select REPLACE(REPLACE(@NewLineSub, '[', ''), ']', ''))
	--DECLARE @CurrentCodeSearch int = 422
...
--**************************************
	...
	DECLARE @StringLength int = len(@LogicString)
	...
	BEGIN --Open 3
		...
		WHILE @StartPosition > 0  --starts at 1
			AND @StartPosition < @StringLength
		

		BEGIN --Open 4
			SET @NextPosition = CHARINDEX(@CurrentSectionWord, @LogicString, @StartPosition);  --search for first match
			...
			IF @NextPosition > 0  --if search term found
				BEGIN --Open 5
					...
					PRINT 'NextPosition = '+convert(varchar(50), @NextPosition)

					SET @MatchSnippet = SUBSTRING(@LogicString, @NextPosition, LEN(@CurrentSectionWord) + 20);
															--take snippet
					PRINT 'MatchSnippet = '+@MatchSnippet

					--new way
			
					IF CHARINDEX(@CurrentSectionWord, @LogicString, @StartPosition) > 0 --added @StartPosition 6/6
					BEGIN --Open 6
					...
					PRINT 'NEW StartPosition = '+convert(varchar(50), @StartPosition)
				END --Close 5
			ELSE
			BEGIN
				SET @StartPosition = 0;
...
--Currently in Loop 2
	...
	Select [CodeSearchID], [ObjectName], [SectionStart], [SectionIdentifier], [SectionNum], [Snippet], [InsertDate], count(*) as Num
	From @SelectTable s
	group by [CodeSearchID], [ObjectName], [SectionStart], [SectionIdentifier], [SectionNum], [Snippet], [InsertDate]
	having count(*) > 1

--Added 6/7-------------------------------------
--get the end
	IF OBJECT_ID(N'tempdb.dbo.#SectionEnd') is not null
		DROP TABLE #SectionEnd
	Select s.*, 'SectionEnd' = isnull(LEAD(SectionStart) OVER (partition by CodeSearchID, ObjectName order by SectionNum) - 1, LEN(@LogicString))
	into #SectionEnd
	From @SelectTable s
--check for dupes
	IF OBJECT_ID(N'tempdb.dbo.#Sections') is not null
		DROP TABLE #Sections
	Select s.*, ROW_NUMBER() OVER (partition by s.CodeSearchID, SectionNum, SectionStart order by  SectionEnd) as 'DupeCount'
	into #Sections
	From #SectionEnd s
--insert dupes
	INSERT INTO [RPT01].Sandbox2.dbo.CodeSearch_ErrorLogs_Sections
		([Server], [CodeSearchID], [ObjectName], [SectionStart], [SectionIdentifier], [SectionNum], [Snippet], [InsertDate], [SectionEnd], [DupeCount])
	Select [Server], [CodeSearchID], [ObjectName], [SectionStart], [SectionIdentifier], [SectionNum], [Snippet], [InsertDate], [SectionEnd], [DupeCount]
	From #Sections s
	Where DupeCount > 1
--delete dupes
	DELETE From #Sections Where DupeCount > 1
-----------------------------------------------
		...
		Select [Server], [CodeSearchID], [ObjectName], [SectionStart], [SectionIdentifier], [SectionNum], [Snippet], [InsertDate], [SectionEnd]
...
PRINT '----------------Finished CodeSearchID '+convert(varchar(50), @CurrentCodeSearch)+'  |  '+@ProcName+'-------------------'

DELETE from @CodeSearches where CodeSearchID = @CurrentCodeSearch
SET @SearchesRemaining = (select count(*) from @CodeSearches)
PRINT 'NUM SEARCHES REMAINING = '+convert(varchar(50), @SearchesRemaining)
	...
	BEGIN --Open 8
		...
		IF @NextPosition > 0  --if search term found
			BEGIN --Open 9
				...
				AND substring(@LogicString, @NextPosition + LEN(@MainSearch), 1) in (' ','$') --omit non-matches via table w/i table (e.g. dbo.Data vs dbo.DataRobot --this was formerly done in the 'aliases' table step but it needs to be up here too)
...
--**************************************
		...
		--DECLARE @CurrentCodeSearch int = 37

		IF OBJECT_ID(N'tempdb.dbo.#AliasIndexes') is not null
			DROP TABLE #AliasIndexes

			--the StopIndex determines where the snippet used to determine the alias ends (if one exists)
			--I reworked this 6/5 and added error logging for any remaining anomalies

		Select v.CodeSearchID, v.SnippetID, 
					 'StartIndex' = CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString), --ends at immediate end of search term
					 'FirstSpace' = CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))),
					 'SecondSpace' = CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))) ),
					 'NewLine' =	 CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) ), --this was originally starting after the first space, but that can cause issues. Changed 6/5
					 'StopIndex' = CASE --if NewLine = StartIndex, StartIndex --added 6/2
										when CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) )
											= CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString)
												then CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString)
										--if (StartIndex < second space < newline) OR newline = 0 then second space  --changed 6/5
										when  CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))) ) 
											between CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString)
												and CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) ) 
										   OR CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) ) = 0
												then CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))) )
										--if (first space < newline < second space) OR second space = 0, newline	--changed 6/5
										when  CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) ) 
											between CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString)))
												and CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))) )
											OR CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString))) ) = 0
												then CHARINDEX('$', Snippet, CHARINDEX(v.SearchString, Snippet) + LEN(v.SearchString) )
										else null END
		into #AliasIndexes
	...
	--LOG ERRORS:
		--drop table Sandbox2.dbo.CodeSearch_ErrorLogs_Aliases
		INSERT INTO Sandbox2.dbo.CodeSearch_ErrorLogs_Aliases (CodeSearchID, SnippetID, StartIndex, FirstSpace, SecondSpace, NewLine, StopIndex, Version, ErrorType, InsertDate)
		SELECT v.*, 'Version' = 1, 'ErrorType' = CASE when dupes.SnippetID is not null then 'dupes'
													  when v.StopIndex is null then 'null StopIndex'
													  else 'zero StopIndex' END --can have multiple
													  , getdate()
		FROM #AliasIndexes v
		left join 
			(select CodeSearchID, SnippetID, count(*) as 'rows'
			 from #AliasIndexes
			 where CodeSearchID = @CurrentCodeSearch
			 group by CodeSearchID, SnippetID
			 having count(*) > 1) dupes on v.CodeSearchID = dupes.CodeSearchID and v.SnippetID = dupes.SnippetID
		WHERE 1=1
		--AND v.CodeSearchID = @CurrentCodeSearch
		AND v.CodeSearchID = @CurrentCodeSearch
		AND 
			(isnull(v.StopIndex,0) = 0
				OR 
			 dupes.SnippetID is not null)

		DELETE ai
		from #AliasIndexes ai
		join Sandbox2.dbo.CodeSearch_ErrorLogs_Aliases err on ai.codesearchID = err.CodeSearchID and ai.SnippetID = err.SnippetID
		...
		select	v.SnippetID
				...
				,'InsertDate' = getdate()
		from Sandbox2.dbo.CodeSearch_Tables_v1 v
		INNER join #AliasIndexes indices on v.SnippetID = indices.SnippetID
		left join Sandbox2.dbo.CodeSearch_Aliases_v1 a (nolock) 
			on v.SnippetID = a.SnippetID
			and v.CodeSearchID = a.CodeSearchID
		WHERE 1=1
	...
	--			,'InsertDate' = getdate()
	--	from Sandbox2.dbo.CodeSearch_Tables_v1 v
	--	join Sandbox2.dbo.CodeSearch_searches_v1 s on v.CodeSearchID = s.CodeSearchID
	...
	--				 'NewLine' =	 CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) ), --this was originally starting after the first space, but that can cause issues. Changed 6/5
	--		--the StopIndex determines where the snippet used to determine the alias ends (if one exists)
	--		--this can have anomalies and needs more work. I think multiple spaces before an alias might throw it off?
	--				 'StopIndex' = CASE --if NewLine = StartIndex, StartIndex --added 6/2
	--									when CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) )
	--										= CHARINDEX(SearchString, Snippet) + LEN(SearchString)
	--											then CHARINDEX(SearchString, Snippet) + LEN(SearchString)
	--									--if (StartIndex < second space < newline) OR newline = 0 then second space
	--									when  CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))) ) 
	--										between CHARINDEX(SearchString, Snippet) + LEN(SearchString)
	--											and CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) ) 
	--									   OR CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) ) = 0
	--											then CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))) )
	--									--if (first space < newline < second space) OR second space = 0, newline
	--									when  CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) ) 
	--										between CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString)))
	--											and CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))) )
	--										OR CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))) ) = 0
	--											then CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) )
	--									else null END
	...
	--	WHERE 1=1
	--		AND a.SnippetID is null
	--		--AND s.LastSearched is null
	--		AND indices.StartIndex >= indices.FirstSpace
	--		--and v.CodeSearchID = 44 --example where NewLine = StartIndex so it was taking 0
	--		and v.CodeSearchID = 216 

	----drop table #temp
	--select CodeSearchID, ObjectName, SearchString, SnippetID, Snippet,
	--				 'StartIndex' = CHARINDEX(SearchString, Snippet) + LEN(SearchString), --ends at immediate end of search term
	--				 'FirstSpace' = CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))),
	--				 'SecondSpace' = CHARINDEX(' ', Snippet, 1 + CHARINDEX(' ', Snippet, (CHARINDEX(SearchString, Snippet) + LEN(SearchString))) ),
	--				 'NewLine' =	 CHARINDEX('$', Snippet, CHARINDEX(SearchString, Snippet) + LEN(SearchString) ), --this was originally starting after the first space, but that can cause issues. Changed 6/5
	...
	--									else null END
	--	into #temp
	--					from Sandbox2.dbo.CodeSearch_Tables_v1
	--					--where CodeSearchID = 216
	--					--where CodeSearchID = 44
	--					where CodeSearchID = 220 and SnippetID in (444,438)

	--	select * from #temp where StopIndex = 0

			--make first stopindex, newline = startindex, newline
...
--insert aliases that still don't exist in the reference table
...
--THIS IS THE NEW/CORRECTED SECTION. I CHANGED IT TO NOT BE DEPENDENT ON THE EXISTENCE OF AN OBJECT, BUT NOW IT'S HANGING. GO BACK AND FIX IT!!!
		...
		--	FROM 
		--		(Select @CurrentAliasID as 'CurrentAliasID') a	
		--	left join [sysobjects] on OBJECT_ID(@ProcName) = [sysobjects].id AND [sysobjects].category = 0
		--	WHERE 1=1		
		--	AND @HasMatch = 1
		...
		----References used in joins, filters, etc:
		--	SELECT 
		--		DISTINCT 
		--		TableSnippetID = (select top 1 SnippetID from @Aliases where AliasID = @CurrentAliasID)
		--		,AliasID = @CurrentAliasID
...
PRINT 'References Runtime (seconds) - '+@Runtime3
...
--You have to update the looped table as well 
	UPDATE @CodeSearches 
...
--Probably faster to get these from the above, but leaving in for qa stuff
